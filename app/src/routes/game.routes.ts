...
export function createGameRouter(gameController : GameController, authMiddleware : AuthMiddleware): Router{
    ...
    gameRouter.post("/", validate(gameSchema.postSchema), authMiddleware.requireAuth, (req, res, next) => gameController.create(req, res, next));

    //PUT /api/games/:id — Actualizar un egame (requiere autenticación)
    gameRouter.put("/:id", validate(gameSchema.putSchema), authMiddleware.requireAuth, (req, res, next) => gameController.update(req, res, next));

    //DELETE /api/games/:id — Eliminar un egame (requiere autenticación)
    gameRouter.delete("/:id", authMiddleware.requireAuth, (req, res, next) => gameController.delete(req, res, next));
