...
export function createStudioRouter(studioController : StudioController, authMiddleware : AuthMiddleware): Router{
    ...
    studioRouter.post("/", validate(studioSchema.postSchema), authMiddleware.requireAuth, (req, res, next) => studioController.create(req, res, next));

    //PUT /api/studios/:id — Actualizar un estudio (requiere autenticación)
    studioRouter.put("/:id", validate(studioSchema.putSchema), authMiddleware.requireAuth, (req, res, next) => studioController.update(req, res, next));

    //DELETE /api/studios/:id — Eliminar un estudio (requiere autenticación)
    studioRouter.delete("/:id", authMiddleware.requireAuth, (req, res, next) => studioController.delete(req, res, next));
